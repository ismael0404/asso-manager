...
<?php if ($detailActivity): ?>
    ...
    <section class="section">
        <div class="container" style="max-width:900px;">
            <a href="<?php echo BASE_URL; ?>/user/activities.php" class="btn btn-secondary btn-sm mb-3"><i
                    class="fas fa-arrow-left"></i> Retour aux activités</a>

            <div class="activity-detail-card">
                <?php if ($detailActivity['image']): ?>
                    <div class="detail-img">
                        <img src="<?php echo BASE_URL . '/assets/images/' . e($detailActivity['image']); ?>"
                            alt="<?php echo e($detailActivity['title']); ?>">
                    </div>
                <?php endif; ?>
                ...
                <div class="detail-body">
                    <div class="detail-badges">
                        <span
                            class="badge badge-<?php echo e($detailActivity['status']); ?>"><?php echo e(translateStatus($detailActivity['status'])); ?></span>
                        ...
                        <?php endif; ?>
                        <span class="participant-count-inline"><i class="fas fa-users"></i> <?php echo $acceptedCount; ?>
                            participant(s)
                    ...
                    <div class="detail-meta">
                        <span><i class="fas fa-calendar"></i>
                            <?php echo date('d/m/Y', strtotime($detailActivity['activity_date'])); ?></span>
                    ...
                    <div class="detail-actions-bar">
                        <?php if (isLoggedIn()): ?>
                            ...
                            <form method="POST" style="display:inline;">
                                ...
                                <button type="submit" class="btn <?php echo $isFav ? 'btn-danger' : 'btn-secondary'; ?>"
                            ...
                            <?php if ($participationStatus === null): ?>
                                ...
                                <?php else: ?>
                                    <span class="participation-status completed-status"><i class="fas fa-ban"></i> Inscriptions non
                                        disponibles</span>
                                <?php endif; ?>
                            <?php elseif ($participationStatus === 'pending'): ?>
                                <div class="participation-status-box pending-box">
                                    <span class="participation-status-label"><i class="fas fa-hourglass-half"></i> ⏳ Demande en
                            ...
                            <?php elseif ($participationStatus === 'accepted'): ?>
                                <div class="participation-status-box accepted-box">
                                    <span class="participation-status-label"><i class="fas fa-check-circle"></i> ✅ Participation
                                        confirmée</span>
                                </div>
                            <?php elseif ($participationStatus === 'rejected'): ?>
                                <div class="participation-status-box rejected-box">
                                    <span class="participation-status-label"><i class="fas fa-times-circle"></i> ❌ Participation
                                        refusée</span>
                                </div>
                            <?php endif; ?>
                        <?php else: ?>
                            <a href="<?php echo BASE_URL; ?>/login.php" class="btn btn-primary btn-lg"><i
                                    class="fas fa-sign-in-alt"></i> Connectez-vous pour participer</a>
                        <?php endif; ?>
                    </div>
                ...
                <?php if (isLoggedIn()): ?>
                    <form method="POST" class="comment-form">
                        ...
                        <div class="comment-input-group">
                            <textarea name="comment_content" class="form-control" placeholder="Écrire un commentaire..."
                    ...
                    <?php foreach ($activityComments as $comment): ?>
                        <div class="comment-item">
                            ...
                            <div class="comment-body">
                                ...
                                <div class="comment-actions-bar">
                                    ...
                                    <?php if (isLoggedIn() && $comment['user_id'] == $_SESSION['user_id']): ?>
                                        <form method="POST" style="display:inline;">
                                            ...
                                            <input type="hidden" name="activity_id" value="<?php echo $detailActivity['id']; ?>">
                                            <button type="submit" class="comment-delete-btn" title="Supprimer"><i
                                ...
                                <?php if (isLoggedIn()): ?>
                                    <div class="reply-form-wrapper" id="replyForm-<?php echo $comment['id']; ?>"
                                        style="display:none;">
                                        <form method="POST" class="reply-form">
                                            ...
                                            <div class="reply-input-group">
                                                <textarea name="comment_content" class="form-control"
                                                    placeholder="Écrire une réponse..." required
                                                    style="min-height:60px;"></textarea>
                                                <button type="submit" class="btn btn-primary btn-sm"><i class="fas fa-reply"></i>
                                                    Répondre</button>
                                            </div>
                                        </form>
                                    </div>
                                                    ...
                                                    <?php if (isLoggedIn() && $reply['user_id'] == $_SESSION['user_id']): ?>
                                                        <form method="POST" style="display:inline;">
                                                            ...
                                                            <input type="hidden" name="activity_id"
                                                                value="<?php echo $detailActivity['id']; ?>">
                                                            <button type="submit" class="comment-delete-btn" title="Supprimer"><i
                                                                    class="fas fa-trash"></i></button>
                                                        </form>
                                                    <?php endif; ?>
            ...
            <div class="activity-filters-bar">
                <form method="GET" class="activity-search-form">
                    <div class="search-box" style="flex:1;">
                        ...
                        <input type="text" name="q" placeholder="Rechercher une activité..."
                ...
                <div class="activity-filter-buttons">
                    <a href="?status=<?php echo $search ? '&q=' . urlencode($search) : ''; ?>"
                        class="btn <?php echo !$statusFilter ? 'btn-primary' : 'btn-secondary'; ?> btn-sm">Toutes</a>
                    <a href="?status=upcoming<?php echo $search ? '&q=' . urlencode($search) : ''; ?>"
                        class="btn <?php echo $statusFilter === 'upcoming' ? 'btn-primary' : 'btn-secondary'; ?> btn-sm"><i
                            class="fas fa-clock"></i> À venir</a>
                    <a href="?status=ongoing<?php echo $search ? '&q=' . urlencode($search) : ''; ?>"
                        class="btn <?php echo $statusFilter === 'ongoing' ? 'btn-primary' : 'btn-secondary'; ?> btn-sm"><i
                            class="fas fa-play"></i> En cours</a>
                    <a href="?status=completed<?php echo $search ? '&q=' . urlencode($search) : ''; ?>"
                        class="btn <?php echo $statusFilter === 'completed' ? 'btn-primary' : 'btn-secondary'; ?> btn-sm"><i
                            class="fas fa-check"></i> Terminées</a>
                </div>
            </div>
            ...
            <?php if (count($activities) > 0): ?>
                            ...
                            <div class="card-body">
                                <div class="mb-1" style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;">
                                    ...
                                    <?php if ($activity['max_participants']): ?>
                                        <span class="participant-count-small"><i class="fas fa-chair"></i>
                                            <?php echo $activity['max_participants']; ?> max</span>
                                    <?php endif; ?>
                                </div>
                                ...
                                <div class="card-meta">
                                    ...
                                    <?php if ($activity['location']): ?>
                                        <span><i class="fas fa-map-marker-alt"></i>
                                            <?php echo e(mb_strimwidth($activity['location'], 0, 30, '...')); ?></span>
                                    <?php endif; ?>
                                </div>
                            </div>
                ...
                <?php if ($totalPages > 1): ?>
                    <div class="pagination mt-4">
                        <?php if ($page > 1): ?><a
                                href="?page=<?php echo $page - 1; ?>&status=<?php echo e($statusFilter); ?>&q=<?php echo urlencode($search); ?>"><i
                                    class="fas fa-chevron-left"></i></a><?php endif; ?>
                        <?php for ($p = 1; $p <= $totalPages; $p++): ?>
                            <?php if ($p == $page): ?><span class="active"><?php echo $p; ?></span>
                            <?php else: ?><a
                                    href="?page=<?php echo $p; ?>&status=<?php echo e($statusFilter); ?>&q=<?php echo urlencode($search); ?>"><?php echo $p; ?></a><?php endif; ?>
                        <?php endfor; ?>
